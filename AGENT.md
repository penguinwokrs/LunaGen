...
## 🧪 テスト
- **ユニット**: Vitest (`*.test.ts` / `*.test.tsx`)。`pnpm test`（watch は `pnpm test:watch`）。
- **実LLM評価ハーネス**: `evals/` 配下。ユニットテストとは別物で、APIキーと課金を伴うため手動実行のみ。
- **プロフィール生成の実LLM評価**: `evals/test-profile-quality.ts`（ヘッダーに実行方法。リポジトリルートから実行）。
