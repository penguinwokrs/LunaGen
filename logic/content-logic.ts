 ...
 * カードを辿る上限ページ数（1ページ8枚）。
 *
 * 2026-08-02 の実測（20件×2腕、同一プロフィールでの対照）で、カードを入れると
 * 生成の PROHIBITED_CONTENT ブロックが 0/20 → 4/20 に増えた。4件とも
 * カード無しでは成功しており、原因はカード側だと確定している。
 * 「首絞め」「メスオナホ」「アナル開発・調教」のような露骨な語が短い羅列として
 * 密集して入るため。自由記述なら文章中に散らばる語が一気に並ぶ形になる。
 *
 * カードはID順（＝おおむねカテゴリ順）で返り、1ページ目は趣味・生活系
 * （写真・カメラが好き / 猫が好き / お酒飲める人がいい 等）が中心で、
 * 2ページ目以降が性的な内容に寄る。1ページに絞ることで露骨な語の密度を下げる。
 */
const MAX_OWN_CARD_PAGES = 1
/** 共通カードも同様に絞る。数十件を羅列してもプロンプトが膨らむだけ */
